...
#if (MBED_CONF_EASY_CONNECT_NETWORK_INTERFACE == WIFI)
...
#if MBED_CONF_EASY_CONNECT_NETWORK_INTERFACE != CELLULAR
...
#else
    if (log_messages) {
        printf("[EasyConnect] %s\n", EASY_STRINGIFY(MBED_CONF_EASY_CONNECT_NETWORK_INTERFACE_CLASS));
    }
    return &iff;
...
static void print_connectivity_messages() {
...
#elif MBED_CONF_EASY_CONNECT_NETWORK_INTERFACE == CELLULAR
    ...
    printf("[EasyConnect] Using Mesh (%s)\n", EASY_STRINGIFY(MBED_CONF_EASY_CONNECT_MESH_RADIO_TYPE));
...
static void setup_connectivity() {
    ...
    ((LoWPANNDInterface*)(&iff))->initialize(&rf_phy);
#elif MBED_CONF_EASY_CONNECT_NETWORK_INTERFACE == MESH_THREAD
    ((ThreadInterface*)(&iff))->initialize(&rf_phy);
#endif
}
